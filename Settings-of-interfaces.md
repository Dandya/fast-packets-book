...
5. [Передача управления сетевым интерфейсом](#передача-управления-сетевым-интерфейсом)
...
Также подсистема `/proc` позволяет настроить то, на каких логических процессорах может обрабатываться прерывание. Эта настройка устанавливается двумя путями:
...
## Передача управления сетевым интерфейсом

Чтобы передать управление сетевым интерфейсом другому драйверу, необходимо выполнить следующие шаги:

1. получение идентификатора сетевого интерфейса;
2. запись идентификатора сетевого интерфейса в файл `unbind` в файловой системе `sysfs` драйвера, от которого необходимо отключить интерфейс;
3. запись идентификатора сетевого интерфейса в файл `bind` в файловой системе `sysfs` драйвера, которому необходимо передать интерфейс.

```bash
# Пример передачи управления WIFI-адаптером драйверу iwlwifi.
# Вывод названий всех подключенных устройств.
lspci -nn
# Запись идентификатора сетевого интерфейса в файл unbind.
echo  "0000:00:14.3" | tee -a /sys/bus/pci/devices/0000\:00\:14.3/driver/unbind
# Запись идентификатора сетевого интерфейса в файл bind.
echo  "0000:00:14.3" | tee -a /sys/bus/pci/drivers/iwlwifi/bind
```

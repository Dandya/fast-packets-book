...
Чтобы перейти от системных вызовов к общей памяти, необходимо с помощью системного вызова `setsockopt`  передать параметры кольцевого буфера командой `PACKET_RX_RING`/`PACKET_TX_RING` на уровне `SOL_PACKET`. По умолчанию создаётся кольцевой буфер первой версии `TPACKET_V1`, но чтобы создать кольцевой буфер более современной версии необходимо также с помощью системного вызова `setsockopt` передать нужную версию командой `PACKET_VERSION` на уровне `SOL_PACKET` [2].

Рассмотрим различия всех трёх версий работы механизма PACKET_MMAP:

1. в версии `TPACKET_V1`:
   - доступны кольцевые буферы RX и TX;
...
Для изучения процесса передачи сетевых пакетов из пространства ядра «Linux» в пользовательское пространство следует обратится к исходному коду системы в файле `contrib/linux-6.18/net/packet/af_packet.c`.
...
// Пример функции по обработке системного вызова recvmsg.
...
// Пример функции по обработке системного вызова sendmsg.
...
static int packet_sendmsg(struct socket *sock, struct msghdr *msg, size_t len)
{
	...
	// Проверка существования кольцевого буфера TX.
	if (data_race(po->tx_ring.pg_vec))
		// Отправка пакетов в кольцевой буфер TX.
...
6. копирование данных пакета в кольцевой буфер RX;
...
```c
// contrib/linux-6.18/net/packet/af_packet.c
// Пример записи пакета в кольцевой буфер RX.
		       ...
		       struct packet_type *pt, struct net_device *orig_dev)
{
	...
	// Копирование данных пакета в кольцевой буфер RX.
...
3. обработка блока:
   ...
   3. заполнение структуры `sk_buff` страницами памяти кольцевого буфера — это позволяет отправлять пакеты без лишнего копирования в буфер структуры `sk_buff`;
   4. обработка ошибки и перемещение в кольцевом буфере;
...
4. установка статуса об ошибке или о результате отправки.

```C
// contrib/linux-6.18/net/packet/af_packet.c
// Пример чтения пакетов из кольцевого буфера TX и их отправка.
static int tpacket_snd(struct packet_sock *po, struct msghdr *msg)
{
	...
	do {
		...
		// Заполнение структуры sk_buff страницами памяти кольцевого буфера,
		...
		// Обработка ошибки и перемещение в кольцевом буфере.
...
out_status:
	// Установка статуса об ошибке или о результате отправки.
...
// Пример функции создания кольцевых буферов 3-й версии.
int
create_rings(int sock_fd, struct rings_buff* rings) {
	...
	// В случае настройки захвата и отправки одновременно кольцевые буферы будут.
...
Чтобы получать любые пакеты, получаемые сетевым интерфейсом, необходимо перевести его в режим «прослушивания». Для этого необходимо использовать системный вызов `setsockopt` с параметрами `SOL_PACKET` и `PACKET_ADD_MEMBERSHIP`, передав значение `PACKET_MR_PROMISC` и индекс сетевого интерфейса.
...
```c
// src/af_packet/rings.c
// Пример обработки пакетов из кольцевого буфера RX.
void
...
```c
// src/af_packet/rings.c
// Пример отправки пакетов через кольцевой буфер TX.
void

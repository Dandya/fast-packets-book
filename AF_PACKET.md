...
3. Получение пакета из блока и его длины;
4. Создание структуры `sk_buff`;
5. Заполнение структуры `sk_buff` страницами памяти кольца --- это, позволяет отправлять пакеты без лишнего
  копирования в буфер структуры `sk_buff`;
6. Обработка ошибки и перемещение в кольце;
7. Обработка блока:
     1. Ожидание новых пакетов;
     2. Установление флага и отправка готового пакета;
     3. Переход к следующему пакету;
     4. Установка статуса об отправке;
8. Установка статуса об ошибке или о результе отправки.

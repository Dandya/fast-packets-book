...
«DPDK» по умолчанию организовывает ожидание пакетов с помощью активного ожидания, то есть непрерывного опроса регистров устройства. Такой метод ожидания сетевых пакетов сильно нагружает ядро. Поэтому в системе доступна работа с прерываниями PCI-устройства.
...
2. определение операций над страницами памяти и их установка;
	...
	int socket_id, const char *ops_name)
{
	...
	// Определение операций над страницами памяти.
	if (mp_ops_name == NULL)
...
1. вызов функции `rte_eth_rx_burst`:
   ...
   3. вызов операции получения пакетов:
      1. заполнение структур `rte_mbuf`:
         ...
         4. увеличение индекса дескриптора пакета;
		 ...
		 struct rte_mbuf **rx_pkts, const uint16_t nb_pkts)
{
	...
	// Сохранение в структуре `rte_mbuf` того, что данные прошли через `RX`
	// очередь.
	       ...
	       uint16_t nb_pkts)
{
	...
	rx_id = rxq->rx_tail;
	// Кольцевой буфер дескрипторов пакетов в драйвере.
	rx_ring = rxq->rx_ring;
	// Кольцевой буфер метаданных пакетов в драйвере.
	sw_ring = rxq->sw_ring;
	// Заполнение структур `rte_mbuf`.
	while (nb_rx < nb_pkts) {
		// Получение указателя на следующий дескриптор.
		rxdp = &rx_ring[rx_id];
		// Проверка того, что дескриптор доступен для чтения.
		...
		rxe = &sw_ring[rx_id];
		// Увеличение индекса дескриптора пакета.
		rx_id++;
...
1. вызов функции `rte_eth_tx_burst`:
   ...
   4. сохранение в структуре `rte_mbuf` того, что данные прошли через `TX` очередь;
   ...
   6. сохранение в структуре `rte_mbuf` не обработанных пакетов того, что очередь `TX` недоступна для отправки пакетов.
		 ...
		 struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	// Сохранение в структуре `rte_mbuf` того, что данные прошли через `TX`
	// очередь.
	...
	// Сохранение в структуре `rte_mbuf` того, что очередь `TX` недоступна
...
4. настройка кольцевых буферов `RX` и `TX` сетевого интерфейса;
...
port_init(uint16_t port, struct rte_mempool *mbuf_pool) {
	...
	for (q = 0; q < rx_queue_count; q++) {
		// Выделение памяти для кольцевого буфера RX и ей настройка.
	...
	for (q = 0; q < tx_queue_count; q++) {
		// Выделение памяти для кольцевого буфера TX и ей настройка.
...
main(int argc, char *argv[]) {
	...
	// Создание именованного кольцевого буфера памяти.
...
Из разбора примеров работы системы «DPDK» и разбора её архитектуры становится ясно, что при захвате пакетов происходит одно копирование пакета механизмом DMA, а при их отправке происходит одно копирование в буфер, который позже читается с помощью DMA сетевой картой. Так как в процессе захвата/отправки сетевых пакетов не требуется использование системных вызовов, ожидание происходит через бесконечный цикл проверки доступных пакетов, а система масштабируется на все логические процессоры, получаем возможность захвата и отправки на скорости выше 100 гигабит в секунду.

Для того, чтобы устройство поддерживалось системой оно должно предоставлять свою внутреннюю память через механизмы UIO или VFIO, определять свою работу на основе настроек внутренней памяти, а также иметь драйвер пользовательского пространства в дереве исходного кода системы «DPDK». Последнее требует для российских компаний, создающих сетевые устройства с новыми чипами, ведения отдельных веток системы, так как добавление драйверов пользовательского пространства в главную ветку системы может быть затруднительным ввиду санкций.

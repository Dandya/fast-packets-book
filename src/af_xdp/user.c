...
// static enum benchmark_type opt_bench = MODE_RXONLY;
static enum xdp_attach_mode opt_attach_mode = XDP_MODE_NATIVE;
static const char *opt_if = "";
static int opt_ifindex;
static int opt_mode;
static int opt_queues_count;
...
static uint32_t opt_pkt_fill_pattern = 0x12345678;
// static bool opt_vlan_tag;
// static uint16_t opt_pkt_vlan_id = VLAN_VID__DEFAULT;
// static uint16_t opt_pkt_vlan_pri = VLAN_PRI__DEFAULT;
...
// 	unsigned long rx_onlyped_npkts;
...
// 	unsigned long prev_rx_onlyped_npkts;
...
// 		printf("AF_XDP statistic for %s:\n", xsk->ifname);
//     printf("\tdropped: %u\n", stats.rx_onlyped);
//     printf("\trx_invalid_descs: %u\n", stats.rx_invalid_descs);
//     printf("\ttx_invalid_npkts: %u\n", stats.tx_invalid_descs);
//     printf("\trx_full_npkts: %u\n", stats.rx_full_descs);
//     printf("\tx_fill_ring_empty_descs: %u\n", stats.rx_fill_ring_empty_descs);
//     printf("\ttx_ring_empty_descs: %u\n", stats.tx_ring_empty_descs);
// 		return 0;
// 	}
...
// 		if (opt_frags) {
// 			uint64_t rx_frags = xsks[i]->ring_stats.rx_frags;
// 			uint64_t tx_frags = xsks[i]->ring_stats.tx_frags;
...
// 				dropped_pps = (xsks[i]->ring_stats.rx_onlyped_npkts -
// 						xsks[i]->ring_stats.prev_rx_onlyped_npkts) *
// 							1000000000. / dt;
...
// 				       xsks[i]->ring_stats.rx_onlyped_npkts);
...
// static void *memset32_htonl(void *dest, uint32_t val, uint32_t size)
// {
...
// 			  __uint32_t len, __uint8_t proto, __wsum sum);
...
// csum_tcpudp_magic(__be32 saddr, __be32 daddr, __uint32_t len,
// 		  __uint8_t proto, __wsum sum)
// {
...
// static uint8_t pkt_data[MAX_PKT_SIZE];
...
// 				  IPPROTO_UDP, (uint16_t *)udp_hdr);
// }

static void gen_eth_frame(struct xsk_umem_info *umem, uint64_t addr)
{
	static uint32_t len;
	...
	memcpy(xsk_umem__get_data(umem->buffer, addr),
			syn_pkt + sizeof(syn_pkt) - len, copy_len);
	len -= copy_len;
}
...
static struct xsk_umem_info *xsk_configure_umem(void *buffer, uint64_t size)
{
	...
	ret = xsk_umem__create(&umem->umem, buffer, size, &umem->pr, &umem->cr,
			       &cfg);
	if (ret)
		exit_with_error(-ret);
...
static void xsk_populate_fill_ring(struct xsk_umem_info *umem)
{
	...
	ret = xsk_ring_prod__reserve(&umem->pr,
				     XSK_RING_PROD__DEFAULT_NUM_DESCS * 2, &idx);
	if (ret != XSK_RING_PROD__DEFAULT_NUM_DESCS * 2)
		exit_with_error(-ret);
	for (i = 0; i < XSK_RING_PROD__DEFAULT_NUM_DESCS * 2; i++)
		*xsk_ring_prod__fill_addr(&umem->pr, idx++) =
...
static void parse_command_line(int argc, char **argv)
{
	...
	load_xdp_prog = true;//(opt_num_xsks > 1 || opt_frags);
	// if (opt_frags)
...
static inline
unsigned int
complete_tx_only(struct xsk_socket_info *xsk, int batch_size)
{
	unsigned int rcvd;
	uint32_t idx;
...
static
void
rx_only(struct xsk_socket_info *xsk)
{
  printf("Rx only\n");
...
static int tx_only(struct xsk_socket_info *xsk, uint32_t *frame_nb,
		   int batch_size, unsigned long tx_ns)
{
	...
	if (opt_tstamp) {
		tv_sec = (uint32_t)(tx_ns / NSEC_PER_SEC);
		tv_usec = (uint32_t)((tx_ns % NSEC_PER_SEC) / 1000);
	}
...
static void load_xdp_program(void)
{
	...
	xdp_prog = xdp_program__open_file("./xdp_kern.o", NULL, NULL);
...
int main(int argc, char **argv)
{
  ...
  num_socks = opt_num_xsks;
	for (i = 0; i < opt_num_xsks; i++)
		xsks[i] = xsk_configure_socket(umem, rx, tx, i);
  ...
  if (opt_mode == MODE_TXONLY)
	...
	if (load_xdp_prog && opt_mode != MODE_TXONLY)
		enter_xsks_into_map();
